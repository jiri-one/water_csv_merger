...
with open(file1) as exp_file, open(file2) as target_file:
    ...
    for target_row in target_reader:
        doctor = Doctor(
            advisor=target_row["Scientific Advisor Name"],
    ...
    for exp_row in exp_reader:
        doctor = Doctor(
            advisor=exp_row["Scientific Advisor Name"],
            title=exp_row["Titul"],
            ...
            if saved_doctor.all_lower == doctor.all_lower:
                ...
                if saved_doctor.address.lower() == doctor.address.lower():
                    ...
                    doctors[index] = replace(
                        ...
                        brick=doctor.brick,
                        originally_selected=(
                            doctor.originally_selected
                            if doctor.originally_selected
                            else saved_doctor.originally_selected
                        ),
                        finally_selected=(
                            doctor.finally_selected
                            if doctor.finally_selected
                            else saved_doctor.finally_selected
                        ),
                        typ=doctor.typ,
...
with open(file3, "w") as csvfile:
    ...
    for doctor in doctors:
        writer.writerow(
            {
                "Scientific Advisor Name": doctor.advisor,
